...
| Status overview                               | ✓         | ✓     |
| Master menu management (association catalog)  |           | ✓     |
| Txosna product selection and overrides        |           | ✓     |
...
- Copying transfers: counter setup, enabled channels, enabled payment methods, selected payment providers, notification modes, QR settings, pending timeout, all TxosnaProduct entries

### Txosna cloning

- Any txosna (not just demo) can be cloned from the txosnak list
- Clone receives new slug, PIN, and starts as CLOSED
- All configuration copied except orders and real-time data
