...
export default function TxosnaProductsPage() {
    ...
    return (
      <div style={{ padding: "32px 32px 60px", background: "var(--adm-bg)", minHeight: "100vh", color: "var(--adm-text-pri)" }}>
        <div style={{ background: "rgba(239,68,68,0.1)", color: "#ef4444", padding: "10px 14px", borderRadius: 8, marginBottom: 16, fontSize: 14, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <button onClick={() => window.location.reload()} style={{ border: "none", background: "transparent", cursor: "pointer", color: "inherit", fontSize: 12, textDecoration: "underline" }}>Berriz saiatu</button>
        </div>
      </div>
    );
  }
  ...
  if (totalProducts === 0) {
    return (
      <div style={{ padding: "32px 32px 60px", background: "var(--adm-bg)", minHeight: "100vh", color: "var(--adm-text-pri)" }}>
        <h1 style={{ fontFamily: "var(--font-nunito, sans-serif)", fontSize: 24, fontWeight: 800, margin: "0 0 24px", color: "var(--adm-text-pri)" }}>Produktuen hautaketa</h1>
        <div style={{ textAlign: "center", padding: "48px 20px", color: "var(--adm-text-sec)", fontSize: 14, border: "1px dashed var(--adm-border)", borderRadius: 12 }}>
          ...
          <h2 style={{ fontSize: 18, fontWeight: 600, marginBottom: 8, color: "var(--adm-text-pri)" }}>
  ...
  return (
    <div style={{ padding: "32px 32px 60px", background: "var(--adm-bg)", minHeight: "100vh", color: "var(--adm-text-pri)" }}>
      {/* Mock Data Banner */}
      {usingMockData && (
        <div style={{ background: "rgba(245,158,11,0.15)", color: "#f59e0b", padding: "10px 14px", borderRadius: 8, marginBottom: 16, fontSize: 14, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 24 }}>
        <div>
          <h1 style={{ fontFamily: "var(--font-nunito, sans-serif)", fontSize: 24, fontWeight: 800, margin: "0 0 6px", color: "var(--adm-text-pri)" }}>Produktuen hautaketa</h1>
          <p style={{ color: "var(--adm-text-sec)", fontSize: 14, margin: 0 }}>
            Aukeratu zein produktu eskaini txosna honetan. Guztira: {enabledProducts} / {totalProducts}
          </p>
        </div>
      </div>
            ...
            >
              ...
              >
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  ...
                  >
                    {category.name}
                    <span style={{ marginLeft: 6, fontSize: 11, opacity: 0.8 }}>({enabledCount}/{category.products.length})</span>
                  </span>
                </div>
                <span
                  style={{
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
        {/* Product Icon */}
        <div style={{ width: 44, height: 44, background: "var(--adm-surface-hi)", borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22, flexShrink: 0 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
            <span style={{ fontSize: 15, fontWeight: 600, color: isEnabled ? "var(--adm-text-pri)" : "var(--adm-text-sec)" }}>
              {product.name}
            </span>
            {product.ageRestricted && <span style={{ fontSize: 10, background: "rgba(239,68,68,0.15)", color: "#ef4444", padding: "1px 6px", borderRadius: 4, fontWeight: 700 }}>+18</span>}
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center", marginTop: 4, flexWrap: "wrap" }}>
            <span style={{ fontSize: 14, fontWeight: 700, color: "#e85d2f" }}>{parseFloat(product.defaultPrice).toFixed(2)} €</span>
            {product.allergens.length > 0 && <span style={{ fontSize: 11, color: "var(--adm-text-dim)" }}>{product.allergens.map(a => ALLERGEN_EMOJI[a] ?? "⚠️").join("")}</span>}
          </div>
          <div style={{ fontSize: 12, color: isEnabled ? "#22c55e" : "var(--adm-text-sec)", marginTop: 4, fontWeight: isEnabled ? 500 : 400 }}>
          ...
          {isEnabled && (hasPriceOverride || hasInstructionsOverride) && (
            <div style={{ display: "flex", gap: 6, marginTop: 6 }}>
              {hasPriceOverride && (
                <span style={{ fontSize: 10, background: "rgba(232,93,47,0.15)", color: "#e85d2f", padding: "2px 6px", borderRadius: 4, fontWeight: 600 }}>
                  💰 Prezioa aldatuta
                </span>
              )}
...
interface EditModalProps {
  ...
  isOpen: boolean;
  onClose: () => void;
  onSave: (productId: string, priceOverride: string | null, preparationInstructions: string | null) => void;
}
...
function EditModal({ product, isOpen, onClose, onSave }: EditModalProps) {
  ...
  const handleSave = () => {
    ...
    onSave(
      product.id,
      priceValue !== null ? String(priceValue) : null,
      instructions || null
    );
    onClose();
  };
  ...
  return (
    ...
    >
      ...
      >
        <h2 style={{ fontFamily: "var(--font-nunito, sans-serif)", fontSize: 20, fontWeight: 800, margin: "0 0 20px", color: "var(--adm-text-pri)" }}>
          {product.name}
        </h2>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          ...
          <div>
            ...
            {product.preparationInstructions && (
              <div style={{ fontSize: 12, color: "var(--adm-text-sec)", marginTop: 6, fontStyle: "italic" }}>
                Katalogo nagusiko argibidea: {product.preparationInstructions.substring(0, 100)}{product.preparationInstructions.length > 100 ? "..." : ""}
              </div>
            )}
          </div>
