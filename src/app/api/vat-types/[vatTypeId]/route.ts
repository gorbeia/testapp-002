...
export async function PATCH(request: NextRequest, { params }: { params: { vatTypeId: string } }) {
...
export async function DELETE(request: NextRequest, { params }: { params: { vatTypeId: string } }) {
  if (!prisma) return new Response('Service unavailable', { status: 503 });
  ...
  const productCount = await prisma.product.count({
    where: { vatTypeId: params.vatTypeId },
  });
