...
describe("TicketCard — basic rendering", () => {
  it("renders order number", () => {
    render(<TicketCard ticket={makeTicket()} isNext={false} onAdvance={vi.fn()} onShowInstructions={vi.fn()} />);
  ...
  it("renders variant detail when detail does not start with Kendu:", () => {
    ...
    render(<TicketCard ticket={ticket} isNext={false} onAdvance={vi.fn()} onShowInstructions={vi.fn()} />);
...
describe("TicketCard — notes", () => {
  ...
  it("does not show notes section when notes is null", () => {
    render(<TicketCard ticket={makeTicket({ notes: null })} isNext={false} onAdvance={vi.fn()} onShowInstructions={vi.fn()} />);
    expect(screen.queryByText("📝")).not.toBeInTheDocument();
  });
});
...
describe("TicketCard — isNext banner", () => {
  it("shows next order banner when isNext=true and status=RECEIVED", () => {
    render(<TicketCard ticket={makeTicket()} isNext={true} onAdvance={vi.fn()} onShowInstructions={vi.fn()} />);
...
describe("TicketCard — action buttons", () => {
  it("advance button label is '→ Hasi' for RECEIVED tickets", () => {
    render(<TicketCard ticket={makeTicket({ status: "RECEIVED" })} isNext={false} onAdvance={vi.fn()} onShowInstructions={vi.fn()} />);
    expect(screen.getByText("→ Hasi")).toBeInTheDocument();
  });

  it("advance button label is '→ Prest' for IN_PREPARATION tickets", () => {
    render(<TicketCard ticket={makeTicket({ status: "IN_PREPARATION" })} isNext={false} onAdvance={vi.fn()} onShowInstructions={vi.fn()} />);
    expect(screen.getByText("→ Prest")).toBeInTheDocument();
  });

  it("calls onAdvance with ticket id when advance button clicked", async () => {
    const onAdvance = vi.fn();
    render(<TicketCard ticket={makeTicket()} isNext={false} onAdvance={onAdvance} onShowInstructions={vi.fn()} />);
  ...
  it("calls onShowInstructions when 📖 button clicked", async () => {
    ...
    render(<TicketCard ticket={makeTicket()} isNext={false} onAdvance={vi.fn()} onShowInstructions={onShowInstructions} />);
