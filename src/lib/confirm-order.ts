...
> {
  ...
  if (!txosna) return { ok: false, status: 404, error: 'Txosna not found' };

  if (associationId !== null && txosna.associationId !== associationId) {
  ...
  const expiresAt = order.createdAt.getTime() + txosna.pendingPaymentTimeout * 60_000;
